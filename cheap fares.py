...
root = Tk()
root.title('Flight Ticket Booking Expedia')
...
def GetWorkSheet():
    ...
    if FROM == TO:
        tkinter.messagebox.showerror("Error", "Source and Destination should be different")
...
##    print "Current date and time using instance attributes:"
    ...
    print(Month)
    print(Year)
    if ( int(years) > int(Year) | (int(years) == int(Year) ) & ( int(months) > int(Month) ) | (int(years) == int(Year) ) & ( int(months) == int(Month) ) & ( int(days) > int(Day) ) ):
...
FlightLabel = """ Expedia Desktop App"""
msg = Message(f0, text = FlightLabel)
msg.config(bg='Red', font=('Book Antiqua', 40, 'italic', 'bold'),width = 1000, relief = RAISED,
...
def GetbestResult(FinalResult):
    ...
    for kl in range (0,len(FinalResult)):
        if FinalResult[kl] == ("Buy"):
            ...
            Invalid += 1
        elif FinalResult[kl] == ("CantDecide"):
            CantDecide += 1
    if (Buy > Wait) and (Buy > UpToYou) and (Buy != Wait) and (Buy != UpToYou) :
##        print "You can BUY..........:-)"
        return "Buy"
...
if len(SelectedCol) > 1:
    ...
    #for the required journey is available.
    #    \n We Suggest You To %s""" % Decision
     driver = webdriver.Chrome('chromedriver.exe')
     driver.get("https://www.expedia.com/")
     eleml = driver.find_element_by_id('Buy')
     eleml.click()
elif Decision == ("Wait"):
    driver = webdriver.Chrome('chromedriver.exe')
    driver.get("https://www.expedia.com/")
    eleml = driver.find_element_by_id('Wait')
    eleml.click()

elif Decision == ("UpToYou"):
    driver = webdriver.Chrome('chromedriver.exe')
    driver.get("https://www.expedia.com/")
    eleml = driver.find_element_by_id('UpToYou')
    eleml.click()
